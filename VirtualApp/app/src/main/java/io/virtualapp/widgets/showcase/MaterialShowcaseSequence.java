...
public class MaterialShowcaseSequence implements IDetachedListener {
	...
	public MaterialShowcaseSequence addSequenceItem(View targetView, String title, String content, String dismissText) {
		...
		MaterialShowcaseView sequenceItem = new MaterialShowcaseView.Builder(mActivity).setTarget(targetView)
				.setTitleText(title).setDismissText(dismissText).setContentText(content).build();
	...
	public void start() {
		...
		if (mSingleUse) {
			...
			/**
			 * See if we have started this sequence before, if so then skip to
			 * the point we reached before instead of showing the user
	...
	public void onShowcaseDetached(MaterialShowcaseView showcaseView, boolean wasDismissed) {
		...
		if (wasDismissed) {
			...
			/**
			 * If so, update the prefsManager so we can potentially resume this
