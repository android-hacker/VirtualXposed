...
public class Orm {
	...
	private Orm(SqLiteConfig config) {
		...
		if (saveDir != null && saveDir.trim().length() > 0) {
			this.db = createDbFileOnSDCard(saveDir, config.getDbName());
		} else {
	...
	public <T> List<T> all(Class<?> clazz) {
		...
		while (cursor.moveToNext()) {
			...
			if (tableInfo.containID) {
				...
				ValueConverter.setKeyValue(cursor, object, tableInfo.primaryField, dataType,
						cursor.getColumnIndex(idFieldName));
			}
	...
	public Orm delete(Class<?> tableClass, Object id) {
		...
		String statement = SQLMaker.deleteByWhere(tableInfo,
				tableInfo.primaryField == null ? "_id" : tableInfo.primaryField.getName() + " = " + idValue);
		try {
			db.execSQL(statement);
	...
	public Orm update(Class<?> tableClass, Object id, Object bean) {
		...
		if (tableInfo.containID) {
			subStatement.append(tableInfo.primaryField.getName()).append(" = ")
					.append(ValueConverter.valueToString(SQLTypeParser.getDataType(tableInfo.primaryField), id));
		} else {
	...
	public <T> T one(Class<?> tableClass, Object id) {
		...
		if (dataType != null) {
			...
			String idValue = ValueConverter.valueToString(dataType, id);
			String statement = SQLMaker.findByWhere(tableInfo,
					tableInfo.primaryField == null ? "_id" : tableInfo.primaryField.getName() + " = " + idValue);
