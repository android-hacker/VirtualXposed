...
public class SQLMaker {
	...
	public static String createTable(TableInfo tableInfo) {
		...
		statement.append("CREATE TABLE ").append("'").append(tableInfo.tableName).append("'").append(" (");

		if (tableInfo.containID) {
			DataType dataType = SQLTypeParser.getDataType(tableInfo.primaryField);
			if (dataType == null) {
				throw new IllegalArgumentException(
						"Type of " + tableInfo.primaryField.getType().getName() + " is not support in WelikeDB.");
			}
			...
			switch (dataType) {
				case INTEGER :
					...
					break;
				default :
					statement.append("  ").append(dataType.name()).append(" PRIMARY KEY");
			}
		...
		for (Field field : tableInfo.fieldToDataTypeMap.keySet()) {
			...
			statement.append("'").append(field.getName()).append("'").append(" ").append(dataType.name());
	...
	public static String insertIntoTable(Object o) {
		...
		if (tableInfo.containID) {
			...
			switch (primaryDataType) {
				...
				default :
					try {
						statement.append(ValueConverter.valueToString(primaryDataType, tableInfo.primaryField, o))
								.append(",");
	...
	public static String findByWhere(TableInfo tableInfo, String where) {
		...
		statement.append(tableInfo.tableName).append(" ").append("WHERE ").append(where);
		...
		for (Field f : tableInfo.fieldToDataTypeMap.keySet()) {
			...
			try {
				builder.append(f.getName()).append(" = ")
						.append(ValueConverter.valueToString(SQLTypeParser.getDataType(f.getType()), f.get(bean)))
						.append(",");
			} catch (Throwable e) {
			}
		}
