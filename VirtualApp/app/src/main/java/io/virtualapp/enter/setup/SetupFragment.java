...
public class SetupFragment extends VFragment<SetupContract.SetupPresenter> implements SetupView {
	...
	public void showAnim() {
		mIconView
				.animate().alpha(1.0f).scaleX(1.2f).scaleY(1.2f).withEndAction(() -> mIconView.animate().scaleX(1.0f)
						.scaleY(1.0f).withEndAction(() -> mGuideBodyLayout.animate().scaleX(1.0f).scaleY(1.0f).start()))
				.start();
	}
}
