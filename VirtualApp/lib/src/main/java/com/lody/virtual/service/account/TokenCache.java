...
public class TokenCache {
	...
	/**
	 * Map associating basic token lookup information with with actual tokens
	 * (and optionally their expiration times).
	 */
	private TokenLruCache mCachedTokens = new TokenLruCache();

	/**
	 * Caches the specified token until the specified expiryMillis. The token
	 * will be associated with the given token type, package name, and digest of
	 * signatures.
	 *
	 * @param token
	 * @param tokenType
	 ...
	 * @param expiryMillis
	 */
	public void put(Account account, String token, String tokenType, String packageName, byte[] sigDigest,
			long expiryMillis) {
	...
	/**
	 * Evicts the specified token from the cache. This should be called as part
	 * of a token invalidation workflow.
	 */
	...
	private static class TokenLruCache extends LruCache<Key, Value> {
		...
		/**
		 * Map associated tokens with an Evictor that will manage evicting the
		 * token from the cache. This reverse lookup is needed because very
		 * little information is given at token invalidation time.
		 */
		...
		protected void entryRemoved(boolean evicted, Key k, Value oldVal, Value newVal) {
			...
			if (oldVal != null && newVal == null) {
				/*
				 * This is recursive, but it won't spiral out of control because
				 * LruCache is thread safe and the Evictor can only be removed
				 * once.
				 */
		...
		public void putToken(Key k, Value v) {
			...
			// Only cache the token once we can remove it directly or by
			// account.
			put(k, v);
		}
